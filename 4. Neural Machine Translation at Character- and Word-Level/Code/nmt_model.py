...
class NMT(nn.Module):
    ...
    def forward(self, source: List[List[str]], target: List[List[str]]) -> torch.Tensor:
        ...
        source_padded_chars = self.vocab.src.to_input_tensor_char(source, device=self.device)   # Tensor: (src_len, b, word_len)
        target_padded_chars = self.vocab.tgt.to_input_tensor_char(target, device=self.device)   # Tensor: (tgt_len, b, word_len)
    ...
    def encode(self, source_padded: torch.Tensor, source_lengths: List[int]) -> Tuple[
        ...
        enc_hiddens, (last_hidden, last_cell) = self.encoder(
            nn.utils.rnn.pack_padded_sequence(X, source_lengths)
        )
        enc_hiddens, lens_unpacked = nn.utils.rnn.pad_packed_sequence(enc_hiddens)
        assert source_lengths == lens_unpacked.tolist()

        enc_hiddens = enc_hiddens.permute(1,0,2)

        init_decoder_hidden = self.h_projection(
            torch.cat((last_hidden[1], last_hidden[0]), dim=1)
        )
        
        init_decoder_cell = self.c_projection(
            torch.cat((last_cell[1], last_cell[0]), dim=1)
        )
    ...
    def decode(self, enc_hiddens: torch.Tensor, enc_masks: torch.Tensor,
        ...
        b, src_len, h = enc_hiddens.shape[0], enc_hiddens.shape[1], int(enc_hiddens.shape[2]/2)
        tgt_len, e = target_padded.shape[0], self.model_embeddings_target.word_embed_size
        # print("\n\nb =", b)
        ...
        assert tuple(enc_hiddens_proj.size()) == (b, src_len, h), "enc_hiddens_proj has wrong dimensions"

        Y = self.model_embeddings_target(target_padded)
        assert tuple(Y.size()) == (tgt_len, b, e), "Y has wrong dimensions"
        ...
        for Y_t in torch.split(Y, 1):
            ...
            o_prev = o_t
        assert len(combined_outputs) == tgt_len, "combined_outputs has wrong length"

        combined_outputs = torch.stack(combined_outputs)
        assert tuple(combined_outputs.size()) == (tgt_len, b, h), "combined_outputs has wrong dimensions"
    ...
    def step(self, Ybar_t: torch.Tensor,
        ...
        assert tuple(dec_hidden.size()) == (b, h), "dec_hidden has incorrect dimensions"

        e_t = torch.bmm(enc_hiddens_proj, torch.unsqueeze(dec_hidden,2))        
        e_t = torch.squeeze(e_t, dim=2)
        assert tuple(e_t.size()) == (b, src_len), "e_t size does not match the prescribed size"
        ...
        a_t = torch.bmm(
            torch.unsqueeze(alpha_t,1), enc_hiddens
        )
        a_t = torch.squeeze(a_t, dim=1)
        assert tuple(a_t.size()) == (b, 2*h), "a_t size does not match the prescribed size"
