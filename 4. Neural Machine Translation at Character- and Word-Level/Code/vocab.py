...
class VocabEntry(object):
    ...
    def to_input_tensor_char(self, sents: List[List[str]], device: torch.device) -> torch.Tensor:
        ...
        padded_sents = pad_sents_char(self.words2charindices(sents), self.char_pad)
        padded_sents = torch.tensor(padded_sents, dtype=torch.long, device=device)
        sents_var = padded_sents.transpose(0, 1)
