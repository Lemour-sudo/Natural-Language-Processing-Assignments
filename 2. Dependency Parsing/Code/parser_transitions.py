...
def minibatch_parse(sentences, model, batch_size):
    ...
    while unfinished_parses:
        ...
        [batch.parse_step(transit) for batch, transit in zip(mini_batch, transitions)]

        unfinished_parses = [
            _item for _item in unfinished_parses 
                    if (_item.buffer or (len(_item.stack) > 1))
        ]

    dependencies = [_item.dependencies for _item in partial_parses]
